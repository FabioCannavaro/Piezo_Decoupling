"""
Ten-fold leave-one-day-out cross-validation for the renamed dataset.

Data structure for each object class:
- 10 actual measurement days
- 30 original measurements per day
- 3 devices × 10 trials per day
- 300 original measurements per class
- filename format: Day01_D1_T01.npy

Each fold uses:
- one day as test
- the next day as validation
- the remaining eight days as training

Training, validation, and test days are mutually exclusive.
Use --folds 1 4 for a pilot run.
Omit --folds for the complete ten-fold run.
"""
...
def parse_original_day_and_block(
    path_or_filename: str,
    samples_per_day: int,
    expected_blocks: int,
) -> Optional[Tuple[int, int]]:
    """
    Extract the actual measurement day from either naming convention.
...
def build_sample_metadata(
    ...
    Original identity is anchored to the filename:
      DayXX_DY_TZZ.npy

    The legacy T1..T300 format is also accepted for backward compatibility.
    ...
    for i, p in enumerate(all_paths):
        key = path_key(p)
        parsed = parse_original_day_and_block(
            p,
        ...
        mapping_rows.append(
            {
                ...
                "expected_samples_per_class": samples_per_day,
                "filename_pattern": f"Day{day:02d}_D*_T*.npy",
            }
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Ten-fold leave-one-day-out training"
    )
    ...
    parser.add_argument("--samples_per_day", type=int, default=30)
    parser.add_argument("--days_per_block", type=int, default=1)
    parser.add_argument("--expected_days", type=int, default=10)
